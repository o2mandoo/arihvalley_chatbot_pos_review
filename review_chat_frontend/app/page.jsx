...
const QUICK_PROMPTS = [
  ...
  '재방문 키워드는 뭐가 있어? 지점별로도 알려줘',
  '지점별로 가장 많이 언급된 불만을 비교해줘',
];

const NEGATIVE_HIGHLIGHT_REGEX =
  /(웨이팅|대기|기다리|시끄럽|복잡|혼잡|늦|느리|오래\s*걸|불친절|아쉽|좁|자리\s*없|비싸|가성비\s*별로|(?:맛|음식|메뉴|서비스|응대|분위기|가격|가성비|청결|위생|양|좌석|자리|공간|소음)\s*(?:이|가|은|는)?\s*별로|별로(?:였|예|네|다|라|야|인|이고|긴|같|함|합니다|했)|(?:^|\s)별로(?=[\s.!?,~]|$)|짰|짠맛|짜요|짜다|짜네|짜서|짜고|간\s*이?\s*(세|쎄)|염도\s*(높|세|쎄)|싱겁|물리)/gi;
  ...
  return (
    <main className="shell">
      <section className="panel sidebar">
        ...
        <div className="card">
          <h2>매출 분석으로 이동</h2>
          <AnimatedNavLink className="switch-btn" href={SALES_CHAT_HREF}>
            매출 분석 챗봇 열기
          </AnimatedNavLink>
        </div>
